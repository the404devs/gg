            ...
            for (let m = 0; m < months.length; m++) {
                ...
                for (let d = 0; d < days.length; d++) {
                    ...
                    if (currentDate >= histStart && currentDate <= histEnd) {
                        $("#" + years[y] + "-" + months[m] + "-" + days[d]).append(
                            $("<button>").addClass("button").attr("onclick", "window.open('./history.html?d=" + years[y] + "-" + months[m] + "-" + days[d] + "')").css("width", "310px").append(
                ...
                monthBlob.children(".month-totals").append(
                    $("<button>").addClass("button").attr("onclick", "window.open('time/gg/" + years[y] + "-" + months[m] + "-GG.pdf')").css("width", "310px").append(
                        $("<span>").html("G&G Timesheet for this Month")
                    )
                ).append(
                    $("<button>").addClass("button").attr("onclick", "window.open('time/bn/" + years[y] + "-" + months[m] + "-BN.pdf')").css("width", "310px").css("margin-left", "5px").append(
